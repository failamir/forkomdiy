<div>
    <div class="card-controls sm:flex">
        <div class="w-full sm:w-1/2">
            Per page:
            <select wire:model="perPage" class="form-select w-full sm:w-1/6">
                @foreach($paginationOptions as $value)
                    <option value="{{ $value }}">{{ $value }}</option>
                @endforeach
            </select>

            @can('data_khusu_delete')
                <button class="btn btn-rose ml-3 disabled:opacity-50 disabled:cursor-not-allowed" type="button" wire:click="confirm('deleteSelected')" wire:loading.attr="disabled" {{ $this->selectedCount ? '' : 'disabled' }}>
                    {{ __('Delete Selected') }}
                </button>
            @endcan

            @if(file_exists(app_path('Http/Livewire/ExcelExport.php')))
                <livewire:excel-export model="DataKhusu" format="csv" />
                <livewire:excel-export model="DataKhusu" format="xlsx" />
                <livewire:excel-export model="DataKhusu" format="pdf" />
            @endif


            @can('data_khusu_create')
                <x-csv-import route="{{ route('admin.data-khusus.csv.store') }}" />
            @endcan

        </div>
        <div class="w-full sm:w-1/2 sm:text-right">
            Search:
            <input type="text" wire:model.debounce.300ms="search" class="w-full sm:w-1/3 inline-block" />
        </div>
    </div>
    <div wire:loading.delay>
        Loading...
    </div>

    <div class="overflow-hidden">
        <div class="overflow-x-auto">
            <table class="table table-index w-full">
                <thead>
                    <tr>
                        <th class="w-9">
                        </th>
                        <th class="w-28">
                            {{ trans('cruds.dataKhusu.fields.id') }}
                            @include('components.table.sort', ['field' => 'id'])
                        </th>
                        <th>
                            {{ trans('cruds.dataKhusu.fields.nama_daerah') }}
                            @include('components.table.sort', ['field' => 'nama_daerah'])
                        </th>
                        <th>
                            {{ trans('cruds.dataKhusu.fields.data_daerah') }}
                            @include('components.table.sort', ['field' => 'data_daerah.nama_daerah'])
                        </th>
                        <th>
                            {{ trans('cruds.dataDaerah.fields.nama_daerah') }}
                            @include('components.table.sort', ['field' => 'data_daerah.nama_daerah'])
                        </th>
                        <th>
                            {{ trans('cruds.dataKhusu.fields.jumlah_anggota_daerah') }}
                            @include('components.table.sort', ['field' => 'jumlah_anggota_daerah'])
                        </th>
                        <th>
                            {{ trans('cruds.dataKhusu.fields.nama_cabang') }}
                            @include('components.table.sort', ['field' => 'nama_cabang'])
                        </th>
                        <th>
                            {{ trans('cruds.dataKhusu.fields.jumlah_anggota_cabang') }}
                            @include('components.table.sort', ['field' => 'jumlah_anggota_cabang'])
                        </th>
                        <th>
                            {{ trans('cruds.dataKhusu.fields.data_cabang') }}
                            @include('components.table.sort', ['field' => 'data_cabang.nama_cabang'])
                        </th>
                        <th>
                            {{ trans('cruds.dataCabang.fields.nama_cabang') }}
                            @include('components.table.sort', ['field' => 'data_cabang.nama_cabang'])
                        </th>
                        <th>
                            {{ trans('cruds.dataKhusu.fields.nama_sub_wilayah') }}
                            @include('components.table.sort', ['field' => 'nama_sub_wilayah'])
                        </th>
                        <th>
                            {{ trans('cruds.dataKhusu.fields.jumlah_anggota_sub_wilayah') }}
                            @include('components.table.sort', ['field' => 'jumlah_anggota_sub_wilayah'])
                        </th>
                        <th>
                            {{ trans('cruds.dataKhusu.fields.data_sub_wilayah_lain') }}
                            @include('components.table.sort', ['field' => 'data_sub_wilayah_lain.nama_wilayah'])
                        </th>
                        <th>
                            {{ trans('cruds.dataWilayah.fields.nama_wilayah') }}
                            @include('components.table.sort', ['field' => 'data_sub_wilayah_lain.nama_wilayah'])
                        </th>
                        <th>
                        </th>
                    </tr>
                </thead>
                <tbody>
                    @forelse($dataKhusus as $dataKhusu)
                        <tr>
                            <td>
                                <input type="checkbox" value="{{ $dataKhusu->id }}" wire:model="selected">
                            </td>
                            <td>
                                {{ $dataKhusu->id }}
                            </td>
                            <td>
                                {{ $dataKhusu->nama_daerah }}
                            </td>
                            <td>
                                @if($dataKhusu->dataDaerah)
                                    <span class="badge badge-relationship">{{ $dataKhusu->dataDaerah->nama_daerah ?? '' }}</span>
                                @endif
                            </td>
                            <td>
                                @if($dataKhusu->dataDaerah)
                                    {{ $dataKhusu->dataDaerah->nama_daerah ?? '' }}
                                @endif
                            </td>
                            <td>
                                {{ $dataKhusu->jumlah_anggota_daerah }}
                            </td>
                            <td>
                                {{ $dataKhusu->nama_cabang }}
                            </td>
                            <td>
                                {{ $dataKhusu->jumlah_anggota_cabang }}
                            </td>
                            <td>
                                @if($dataKhusu->dataCabang)
                                    <span class="badge badge-relationship">{{ $dataKhusu->dataCabang->nama_cabang ?? '' }}</span>
                                @endif
                            </td>
                            <td>
                                @if($dataKhusu->dataCabang)
                                    {{ $dataKhusu->dataCabang->nama_cabang ?? '' }}
                                @endif
                            </td>
                            <td>
                                {{ $dataKhusu->nama_sub_wilayah }}
                            </td>
                            <td>
                                {{ $dataKhusu->jumlah_anggota_sub_wilayah }}
                            </td>
                            <td>
                                @if($dataKhusu->dataSubWilayahLain)
                                    <span class="badge badge-relationship">{{ $dataKhusu->dataSubWilayahLain->nama_wilayah ?? '' }}</span>
                                @endif
                            </td>
                            <td>
                                @if($dataKhusu->dataSubWilayahLain)
                                    {{ $dataKhusu->dataSubWilayahLain->nama_wilayah ?? '' }}
                                @endif
                            </td>
                            <td>
                                <div class="flex justify-end">
                                    @can('data_khusu_show')
                                        <a class="btn btn-sm btn-info mr-2" href="{{ route('admin.data-khusus.show', $dataKhusu) }}">
                                            {{ trans('global.view') }}
                                        </a>
                                    @endcan
                                    @can('data_khusu_edit')
                                        <a class="btn btn-sm btn-success mr-2" href="{{ route('admin.data-khusus.edit', $dataKhusu) }}">
                                            {{ trans('global.edit') }}
                                        </a>
                                    @endcan
                                    @can('data_khusu_delete')
                                        <button class="btn btn-sm btn-rose mr-2" type="button" wire:click="confirm('delete', {{ $dataKhusu->id }})" wire:loading.attr="disabled">
                                            {{ trans('global.delete') }}
                                        </button>
                                    @endcan
                                </div>
                            </td>
                        </tr>
                        @empty
                        <tr>
                            <td colspan="10">No entries found.</td>
                        </tr>
                    @endforelse
                </tbody>
            </table>
        </div>
    </div>

    <div class="card-body">
        <div class="pt-3">
            @if($this->selectedCount)
                <p class="text-sm leading-5">
                    <span class="font-medium">
                        {{ $this->selectedCount }}
                    </span>
                    {{ __('Entries selected') }}
                </p>
            @endif
            {{ $dataKhusus->links() }}
        </div>
    </div>
</div>

@push('scripts')
    <script>
        Livewire.on('confirm', e => {
    if (!confirm("{{ trans('global.areYouSure') }}")) {
        return
    }
@this[e.callback](...e.argv)
})
    </script>
@endpush